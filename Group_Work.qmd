...
We would appreciate some comments on (1) the reproducibility of our code and whether this could be improved, and whether there are any coding conventions it would be helpful to be aware of to produce a professional product in future, and (2) any improvements that could be made to the way we have tangled code and prose; the large code chunks in particular, while they seem necessary, do make it difficult to focus on the flow of the narrative when putting the report together and the difficulty of confirming the wordcount of the overall document has made the drafting process somewhat more complicated than perhaps it needed to be.
...
The opposition's proposal is likely to gain some traction due to public awareness of recent protests and measures taken against Airbnb in other holiday destinations [@hedgecoe2025], and a perception (due to repeated promises by more than one UK government to build more housing) that there is a shortage of housing in the UK [@mhclg2024]. The discovery of the Mayor's aide's housing circumstances has stirred media interest, feeding into the debate about the housing market and suspicions of political hypocrisy [@mckiernan2025].

While studies have investigated the undesirable consequences for areas that host large proportions of Airbnb properties, such as increased crime rates and anti-social behaviour [@lanfear2024promise], the opposition's proposal focuses on Airbnb's impact on housing stock. This report does likewise by drawing on data from Inside Airbnb[^1], to evaluate the scale and nature of Airbnb's penetration into the London property market, and make recommendations on how to manage both Airbnb's impact, and the fallout from the allegations against the aide.

[^1]: Inside Airbnb obtains public information on rentals available for booking at specific months. We discuss the limitations of the data at the end of this report. 
...
3.  The opposition's proposed council tax surcharge is a blunt and politically difficult tool. Effective alternatives include implementing landlord registration, enforcement of business rates for commercial operators or a tourist tax, to target problematic hosts without disadvantaging ordinary residents who rely on modest hosting income.
4.  The aide's conduct may be subject to criminal sanction and could certainly undermine the Mayor's otherwise positive record on this issue. It should be investigated fully but not allowed to dominate the conversation around the opposition's proposal and its deficiencies, or the alternative courses of action.

[^2]: Using the statutory definition of inner / outer London boroughs: [statutory definition](https://www.legislation.gov.uk/uksi/1965/540/article/2/made).
...
```{python}
# Set overall font for chart plots
...
Since 2015, London landlords letting their property for more than 90 days per year have been required to obtain planning permission (which can be denied) for a change of use [@mayoroflondonn.d.]. More recently, the Mayor has proposed both a landlord registration system [@mayoroflondon2019] and a licensing system [@mayoroflondon2023]. Airbnb and other equivalent platforms are therefore under far more "control" in London than in many European cities.

Having said that:

-   It is true that the growth of Airbnbs has far outpaced that of housing stock in London in recent years.

-   It may be the case that a large proportion of hosts are flouting the planning permission requirement, although we have not been able to establish an exact figure at this stage (see Limitations section below). One course of action might be to investigate this point and take the appropriate enforcement action against landlords in breach. However, this process is of course not costless.
...
Most of the increase occurred between 2022-2023, with new listings concentrated in inner London:

-   **`{python} f"{top5_boros[0]}"`**, **`{python} f"{top5_boros[1]}"`**, **`{python} f"{top5_boros[2]}"`**, **`{python} f"{top5_boros[3]}"`** and **`{python} f"{top5_boros[4]}"`** saw the highest increase in listings.
...
Across all London Boroughs in 2025, the average number of listings as a percentage of housing stock was **`{python} f"{BORO_mean:.1f}%"`**, though **`{python} f"{BORO_above_mean_n} Boroughs"`** exceeded this average. These **`{python} f"{BORO_above_mean_n} Boroughs"`** were in inner London. Close to **`{python} f"{top1_listingprop_value:.0f}%"`** of total housing stock in **`{python} f"{top1_listingprop_name}"`**, and **`{python} f"{top2_listingprop_name}"`** were listed as an Airbnb ([Figure 3]{.underline}). Up to **1 in 8 homes** in some MSOAs in inner London were listed on Airbnb ([Figure 4]{.underline}).
...
[^4]: Some of the hosts letting on a "frequent let" basis may have complied with the planning permission requirement. To establish how many hosts fall into this category, it would be necessary to obtain the number of permissions from the Planning London Datahub’s public API as an estimate and conduct time-intensive work to associate the permissions with the relevant Airbnb properties by building a text regex pattern to search for the properties within the planning permission descriptions.
...
Notwithstanding the sensitivity of assumptions[^5], we expect this to be a conservative estimate, given the reported likelihood of multiple listings of the same property, which some owners have used to circumvent the 90 day rule [@benjamin2025].
...
The opposition proposes that "professional" landlords' properties should be subject to registration and increased council tax. The Mayor's office should make clear in any response that it has already proposed a registration system for landlords, and welcome the opposition's belated support.

The opposition has not defined "professional" landlord, nor provided details of how their proposal would be implemented.
...
Studies suggest that a company or individual that owns and operates [two or more entire homes]{.underline} can be classed as a commercial or professional landlord. Such "multi-listing hosts" could be drivers of increase in rent and of Airbnb's profit in London [@todd2021; @cox2016].
...
-   **`{python} f"{df_only1_hosts:,} ({df_only1_hosts_perc:.0f}%)"`** hosts own [only 1 entire property ("single-listing hosts")]{.underline}, and are responsible for **`{python} f"{df_only1_perc:.0f}%"`** of listings.
...
Based on estimated revenues[^7],

[^7]: Estimated revenues obtained from Inside Airbnb, computed as a function of price and occupancy estimates. The median was used as providing a more accurate measure of central tendency for revenues, given the skew created by luxury homes.

-   The median single-listing host renting out on "standard lets" earned **£`{python} f"{median_income_single_90d:,.0f}"`** a year per property, while the median multi-listing host earned **£`{python} f"{median_income_multi_90d:,.0f}"`**.

-   The median single-listing "frequent lets" host earned **£`{python} f"{median_income_single_90dmore:,.0f}"`** a year per property, while multi-listing "frequent lets" hosts earned **£`{python} f"{median_income_multi_90dmore:,.0f}"`**.
...
Using the more conservative income from single-listing hosts on standard lets, **£`{python} f"{median_income_single_90d:,.0f}"`**, **`{python} f" owning {math.floor(single_listing_host_90d):,} properties"`** becomes the threshold to make more than the London median income.

However, with frequent lets, **`{python} f"owning {math.ceil(single_listing_host_90dmore):,} property"`** is sufficient to make close to double the median income of London. Just **two properties** can generate nearly double the median income of London. Therefore, categorising any host owning two or more entire properties as a “professional” landlord is a defensible position.

If the opposition’s proposal were implemented using this definition of "professional" hosts, **`{python} f"{len(df_morethan1):,}"`** properties would be subject to increased council tax, constituting **`{python} f"{df_morethan1_perc:.0f}%"`** of all entire homes listings.

These figures do not account for the operating costs that a landlord must pay in relation to their propertie, which reduce the bottom line for a landlord, especially where they own fewer properties and cannot benefit from economies of scale.
...
Most of both single **`{python} f"({only1prop_UK_per:.0f}%)"`** and multi-listing **`{python} f"({propmorethan1_UK_per:.0f}%)"`** hosts are UK-based. However, at least one of the hosts who owns over 100 properties is a company based abroad (Dubai). Three others have names suggesting that they are companies. However, these numbers are incomplete because **`{python} f"{host_NA:.0f}%"`** of hosts do not provide a host location; a dialogue with Airbnb around their registration process may be beneficial.

## 3. Evaluating the opposition's proposal

#### [Is council tax an appropriate additional control?]{.underline}

The viability of the opposition’s proposal relies on the benefits of increasing council tax outweighing any negative impacts of the high proportion of Airbnb properties in London. It is not clear that this would be the effect.

Council tax applies to a property’s domestic classification, based on its 1991 valuation. All properties in the same band pay the same rate of council tax, unless an exemption applies. It is not a mechanism designed to assess current commercial use or income, making it a blunt instrument for targeted market intervention.

The opposition do not explain how the council tax system might be adjusted. However, we note that the recent 2025 Budget outlines plans to modify the council tax system from 2028 to apply a surcharge on luxury properties, using 2026 valuations[@mcnamee2025]. This suggests greater flexibility in the system than has historically been possible, and further work to “piggyback” on these changes could be explored.

To illustrate potential revenues from council tax adjustments, we calculated the estimated revenue increase based on a **2% - 10%** **premium** applied to the current council tax Band D rate[^8] for all properties operated by "professional" landlords.

[^8]: Band D was chosen as it sits in the middle of valuation bands and best represents a typical tax without unfairly skewing towards low or high property values. This is aligned with methodology used by councils for statutory tax base computations [@mhclg2025a].
...
Note: A conservative range of 2%, 5%, and 10% was chosen for simulation, given the requirement that councils proposing a \>5% increase must hold a successful local referendum [@mhclg2025b].
...
The spatial concentration of potential revenue could allow the Mayor to justify the policy as one specifically taxing central areas, where commercial exploitation of housing stock is most acute, rather than a blanket tax rise across London ([Figure 7]{.underline}). 
...
Given that some of the existing Airbnb hosts are companies, and that a great deal more would be required to register as “professional” landlords if some form of these proposals were carried out, [business rates]{.underline} might be a viable targeted alternative that relies on an existing regulatory infrastructure rather than creating a parallel scheme. These rates must be paid when a property is used for commercial purposes for more than the prescribed number of days, typically 140 per year [@gov.uk]. This would have the benefit of clarity, as "professional" hosts would be classed as businesses and there would be no need to set up a separate infrastructure. However, the requirement to register for business rates is based on different thresholds, and some work to align these with the planning permission requirements would be required.

Another option is a “[tourist tax]{.underline}”, a common approach in other countries. The Hackney Citizen estimates that such a measure could raise **£240 million a year** and, if implemented with care, would not necessarily reduce visitor numbers [@jaffer2025]. This would be more targeted and easily administered, without relying on property valuations or enforcement against reluctant or elusive landlords, since it is typically paid by the tourist.

## 4. Weighing up the way ahead: pros, cons and social mobility

The opposition’s proposal is not a wholly bad development and provides an opportunity to emphasise previous steps taken and ride the wave of bipartisan support to implement more progressive tax changes than those the opposition suggests.

However, too heavy-handed an approach risks undermining London’s thriving tourism industry, which contributes at least **£10 billion a year** to London’s economy and supports around **7% of London’s jobs** [@mayoroflondon2025]. Moreover, London's short letting market is already more tightly regulated than many European equivalents, and overcorrection should be avoided ([Figure 8]{.underline}).
...
Furthermore, introducing additional administrative complexity and costs through broad council tax increases would reduce opportunities for social mobility.

For ordinary Londoners, hosting often provides a meaningful supplement to household income, helping them manage rising living costs and maintain financial stability. For example, older women living alone and women with disabilities increasingly rely on hosting as a flexible lifeline in London [@remap2022]. For many renting out a single room, earnings are modest: the median annual revenue for a private room let under 90 days is **`{python} f"£{median_home_standard:,.0f}"`** rising to around **`{python} f"£{median_home_frequent:,.0f}"`** for frequent lets; some of these hosts may already be paying income tax on their earnings, in addition to operating costs. 

While better-off “professional” hosts could absorb the additional burden without necessarily releasing their properties onto the housing market, individual hosts may see the benefit of letting their property diminished to the point of exiting the market, to the detriment of their finances and security.

Rather than a blanket approach such as raising council tax for all, a targeted strategy, which could combine a tourist tax with a focus on ensuring that professional landlords are registered and paying business rates would be more proportionate, effective, and better aligned with the Mayor’s aim of reducing harm, without compromising London’s vibrancy or the livelihoods of ordinary residents.

## 5. The aide: deflecting a distraction

Despite the potential upsides described above to the opposition raising this issue, the aide's conduct is, at the least, a source of  embarrassment for the Mayor's office and, at worst, a criminal offence (under the Prevention of Social Housing Fraud Act 2013).  The appropriate steps must be taken so that public trust in the Mayor's office can be maintained, following the usual protocols around internal investigations.

However:

-   The aide's full circumstances are not known and any criminal prosecution is a matter for the CPS; in some circumstances it is permitted to let social housing [@shelter].

- Even for multi-property owners, this analysis shows that some hosts may require up to seven properties to reach the London median income. Although the aide is letting two properties, their income may not meet that threshold. 

The opposition is clearly using the aide's circumstances to score political points, but in doing so have put themselves in the position of having to support previous initiatives suggested by the Mayor's office: an opportunity that should not be missed. They should not be permitted to use the aide's alleged misdemeanours to distract from the more salient issues around Airbnb's impact on London, or to suggest that individuals should be barred from legitimate ways of boosting their income for greater security.

## 6. Limitations

#### [**Inside Airbnb dataset**]{.underline} [@insideairbnb] 

There are several limitations to this dataset, other than those noted above:

-   Inaccuracy in web-scraping may lead to listing numbers being underestimated during the data cleaning process. We further processed the data to remove listings with duplicate IDs, or more than six null fields, which typically reflects the fact that entries have no reviews or incomplete information.

-   The dataset is only a snapshot in time. Listings can be added or deleted and seasonal/temporary listings might be over/under- represented. 


#### [**Housing stock dataset**]{.underline} [@valuationofficeagency2024]

Using a dataset containing the raw stock of properties to normalise Airbnb listings might overlook variations in housing stock. Some areas may have less Airbnb-eligible dwellings (eg. care homes) and some may have higher vacancy rates than others.  
