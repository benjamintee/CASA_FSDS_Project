...
-   **£`{python} f"{median_income_single_90d:,.0f}"`** for single-listing hosts and **£`{python} f"{median_income_multi_90d:,.0f}"`** for multi-listing hosts on"standard lets".

-   **£`{python} f"{median_income_single_90dmore:,.0f}"`** for single-listing hosts and **£`{python} f"{median_income_multi_90dmore:,.0f}"`** for multi-listing hosts on "frequent lets".
...
If the opposition’s proposal were implemented using this definition, **`{python} f"{len(df_morethan1):,}"`** properties would be subject to increased council tax, **`{python} f"{df_morethan1_perc:.0f}%"`** of all entire homes listings.

These figures do not account for a landlord's operating costs, which reduces bottom lines, especially where they own fewer properties and cannot benefit from economies of scale.
...
Most of both single **`{python} f"({only1prop_UK_per:.0f}%)"`** and multi-listing **`{python} f"({propmorethan1_UK_per:.0f}%)"`** hosts are UK-based. However, at least one of the hosts that owns over 100 properties is a company based abroad (Dubai). Three others have names suggesting that they are companies. However, these numbers are incomplete because **`{python} f"{host_NA:.0f}%"`** of hosts do not provide a host location; a dialogue with Airbnb around their registration process may be beneficial.

## 3. Evaluating the opposition's proposal

#### [Is council tax an appropriate additional control?]{.underline}

It is not clear that revenues from increasing council tax would outweigh the negative impact of the high proportion of Airbnb lettings in London.
