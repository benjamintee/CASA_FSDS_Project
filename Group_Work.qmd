...
We would appreciate comments on (1) the reproducibility of our code and whether this could be improved, and recommended coding conventions that would be helpful to produce a professional product in future, (2) any improvements that could be made to the way we have tangled code and prose; the large code chunks in particular, while they seem necessary, do make it difficult to focus on the flow of the narrative when putting the report together, and (3) any quick ways to check wordcount, as the difficulty of confirming the wordcount of the overall document has made the drafting process slightly more complicated.
...
The opposition's proposal will likely gain some traction due to public awareness of measures taken against Airbnb in other holiday destinations [@hedgecoe2025], and of shortage of housing in the UK [@mhclg2024]. The Mayor's aide's circumstances have stirred media interest, feeding into the debate about the housing market and suspicions of political hypocrisy [@mckiernan2025].

While studies have investigated the undesirable consequences for areas that host large proportions of Airbnb properties, such as increased crime rates [@lanfear2024promise], the opposition's proposal focuses on Airbnb's impact on housing stock. This report does likewise by using data from Inside Airbnb,[^1] to evaluate Airbnb's penetration into the London property market, and make recommendations on how to manage Airbnb's impact, and the allegations against the aide.

[^1]: Inside Airbnb obtains public information on rentals available for booking at specific months. See Limitations section below.

In summary:

1.  London's Airbnb listings have expanded faster than housing supply, particularly in inner boroughs,[^2] likely heightening market pressures and public concern.
...
3.  A council tax surcharge is a blunt and politically difficult tool. There are effective alternatives that would target problematic hosts without disadvantaging ordinary residents who rely on modest hosting income.
4.  The aide's conduct may be subject to criminal sanction and could undermine the Mayor's record. It should be investigated fully but not allowed to dominate the conversation around the opposition's proposal and its deficiencies, or the alternative courses of action.

[^2]: Using the [statutory definition](https://www.legislation.gov.uk/uksi/1965/540/article/2/made) of inner / outer London boroughs.
...
```{python}
#| label: Set overall font for chart plots
#| echo: false
#| warning: false
...
```{python}
#| label: Download Airbnb listings data
#| echo: false
#| warning: false
#| output: false
...
```{python}
#| label: Cleaning the Airbnb listings data (2021-2025) 
#| echo: false
#| warning: false
#| output: false
...
```{python}
#| label: Cleaning the data to remove NAs appropriately 
#| echo: false
#| warning: false
#| output: false
for year, df in dfs.items():
...
```{python}
#| label: Assign correct variable types to each column for efficiency in processing 
#| echo: false
#| warning: false
#| output: false
# Columns to convert
...
```{python}
#| label: Load MSOA 2021 geopackage and BORO files from GitHub
#| echo: false
#| warning: false
#| output: false
...
The opposition are behind the times: the increase in short-term lets and its potential to damage the housing supply is a concern that the Mayor's office has already raised and taken concrete steps to tackle.

Since 2015, London landlords letting their property for more than 90 days per year must obtain planning permission for a change of use [@mayoroflondon2023-1]. The Mayor has also proposed a landlord registration system [@mayoroflondon2019] and a licensing system [@mayoroflondon2023]. Airbnb and its competitors are therefore under far more "control" in London than in many other cities [@bei2023].

Having said that:

-   The growth of Airbnbs has recently far outpaced that of housing stock in London.

-   A large proportion of hosts may be flouting the planning permission requirement, although exact numbers are difficult to establish (see Limitations section). Once established, appropriate action could be taken against landlords in breach. However, this process is not costless.

#### [Changes in Airbnb listings and housing stock 2021-2025]{.underline}

```{python}
#| label: Key statistics for growth trend in listings 2021-2025
#| echo: false
#| warning: false
#| output: false
listings_2021 = round(len(dfs["2021"]),-2)
listings_2025 = round(len(dfs["2025"]),-2)
...
Since 2021, London has seen a **`{python} f"{listings_pct_change:.0f}%"`** increase in Airbnb listings, rising from **`{python} f"{listings_2021:,}"`** to **`{python} f"{listings_2025:,}"`** listings ([Figure 1]{.underline}).

```{python}
#| label: Increase in Airbnb listings by Borough 2021-2025
#| echo: false
#| warning: false
...
```{python}
#| label: Generate growth rates for top 5 boroughs
#| echo: false
#| warning: false
...
rounded_top5_abs = [round(x, -2) for x in top5_abs]
top5_pct   = top5["pct_change"].round(0).tolist()
top5_2025  = top5["2025"].tolist()
```

Most of this occurred between 2022-2023, with new listings concentrated in inner London:

-   **`{python} f"{top5_boros[0]}"`**, **`{python} f"{top5_boros[1]}"`**, **`{python} f"{top5_boros[2]}"`**, **`{python} f"{top5_boros[3]}"`** and **`{python} f"{top5_boros[4]}"`** saw the highest increase.

-   **`{python} f"{top5_boros[0]}"`** alone added **`{python} f"{rounded_top5_abs[0]:,}"`** listings, a **`{python} f"{top5_pct[0]:.0f}%"`** increase.

```{python}
#| label: Download data from Valuation Office Agency on Housing Stock 
#| echo: false
#| warning: false
#| output: false
...
```{python}
#| label: Create a stock_table detailing the total number of properties in London each year 
#| echo: false
#| warning: false
stock_table = (
    df_stock_MSOA
...
```{python}
#| label: Change in stock and listings at the Borough level
#| echo: false
#| warning: false
df_stock_LA_wide = (
    df_stock_LA
...
In contrast, between 2020 and 2024, London's housing stock grew from **`{python} f"{stock_2020 / 1000000:.2f} million"`** to **`{python} f"{stock_2024 / 1000000:.2f} million"`** properties: a much smaller increase of **`{python} f"{stock_growth:.1f}%"`**. **`{python} f"{largest_ratio_name}"`** saw **`{python} f"{largest_ratio_value} times"`** more new Airbnb listings than new housing stock.
...
```{python}
#| label: Aggregation of listings to Boroughs by count
#| echo: false
#| warning: false
...
Across London boroughs, the average number of listings as a percentage of housing stock was **`{python} f"{BORO_mean:.1f}%"`**, though **`{python} f"{BORO_above_mean_n} inner London boroughs"`** exceeded this average. Almost **`{python} f"{top1_listingprop_value:.0f}%"`** of total housing stock in **`{python} f"{top1_listingprop_name}"`**, and **`{python} f"{top2_listingprop_name}"`** were listed on Airbnb ([Figure 3]{.underline}). Up to **1 in 8 homes** in some MSOAs in inner London were listed on Airbnb ([Figure 4]{.underline}).
...
In a tight housing market, this exacerbates pressure on housing prices and rents [@todd2021], driving a perception that Airbnb could be “out of control” in parts of London.
...
ax.set_ylabel("Listings as % of Housing Stock")
...
```{python}
#| label: Aggregation of listings to MSOA level 
#| echo: false
#| warning: false
# Filter the total stock data for 2024
...
```{python}
#| lst-label: Figure 4 - Airbnb Listings as a Proportion of Housing Stock
#| echo: false
#| warning: false
...
```{python}
#| label: Compute the number of properties and percentage in each category in 2025 
#| echo: false
#| warning: false
df_2025 = dfs['2025'] 
type_summary_2025 = (
...
entire_homes_2025_90_n = round(len(entire_homes_2025_90), -2)
entire_homes_2025_90_per = round(entire_homes_2025_90_n / entire_homes_2025_n * 100, 2)
```

```{python}
#| label: Average percentages of entire homes / apartments let out for more than 90 days by Borough 
#| echo: false
#| warning: false
...
Based on occupancy estimates,[^3] **`{python} f"{entire_homes_2025_90_n:,} entire homes"`** were used for 90 or more nights per year ("frequent lets"). This represented **more than 1 in 5 of all entire homes listings**, and **1 in 8 of all listings in London.**[^4]

[^3]: Occupancy estimates obtained from Inside Airbnb [@insideairbnb]. The review rate was then used to convert reviews to estimated bookings and the average length of stay for London was multiplied by the estimated bookings for each listing, giving the occupancy rate (out of 365 days). The estimate of potential breaches was calculated using the subset of listings that are "entire homes" and the occupancy rate, which provides a useful proxy for annual use by estimating how many nights the listing was booked in the last 365 days, where raw daily calendar data are unavailable.

[^4]: Some hosts letting on a “frequent let” basis may comply with the planning permission requirement. Since “short term lets” are not an independent category of planning permission that can be filtered for, estimating how many fall into this category would require obtaining the number of permissions from the Planning London Datahub’s public API and filtering by building a text regex pattern to search within planning permission descriptions. This is time intensive and has not been possible during the course of this analysis.
...
Notwithstanding the sensitivity of assumptions,[^5] we expect this to be a conservative estimate, given reports of multiple listings of the same property [@benjamin2025].

[^5]: The occupancy model assumed (a) a review rate of 50% [@iaassumptions] (Inside Airbnb notes that Airbnb has previously cited an average review rate of over 70%) and (b) average length of stay of 3.1 days for London [@ons2025].

## 2. Taking back "control"?

The opposition proposes that "professional" landlords' properties should be subject to registration and increased council tax. The Mayor's office should make clear that it has already proposed a registration system for landlords, and welcome the opposition's belated support.

The opposition has not defined "professional" landlord, nor provided a plan for implementation.

-   Our analysis provides a reasoned basis for classing any host letting out two or more entire homes on a "frequent let" basis as a professional landlord (note that this would include the aide at the centre of the current controversy).

-   On this basis, the opposition's proposal would mean that over half of London's entire homes lettings would be subject to increased council tax.

#### [What is a professional landlord?]{.underline}

Studies suggest that a company or individual that operates [two or more entire homes]{.underline} can be classed as a professional landlord. Such "multi-listing hosts" could be drivers of increases in rent and of Airbnb's profit in London [@todd2021; @cox2016].

```{python}
#| label: Calculate statistics for hosts 
#| echo: false
#| warning: false
host_sizes = (
    entire_homes_2025
...
# Computing the number of hosts in each group and percentage 
...
-   **`{python} f"{df_only1_hosts:,} ({df_only1_hosts_perc:.0f}%)"`** hosts own [only 1 entire property ("single-listing hosts")]{.underline}, and make up **`{python} f"{df_only1_perc:.0f}%"`** of listings.

-   **`{python} f"{df_morethan1_hosts:,} ({df_morethan1_hosts_perc:.0f}%)"`** hosts own [two or more entire properties ("multi-listing hosts")]{.underline}, and make up **`{python} f"{df_morethan1_perc:.0f}%"`** of listings.

-   **`{python} f"{df_morethan100_hosts:,} ({df_morethan100_hosts_perc:.2f}%)"`** hosts, [own over 100 entire properties,]{.underline} and, between them, **`{python} f"{df_morethan100_perc:.0f}%"`** of listings.

-   Notably, a larger proportion of properties belonging to multi-listings hosts were let out for 90 or more days ("frequent lets") **`{python} f"({df_morethan1_90more_perc:.0f}%)"`**, than properties belonging to single-listing hosts **`{python} f"({df_only1_90more_perc:.0f}%)"`**.
...
# Figure title 
fig.suptitle("Figure 5: A small number of hosts were responsible for a \ndisporportionately large number of listings",
    x= 0.02, y = 0.95,
    fontweight="bold",
    ha="left")
...
Therefore, using the two-property threshold, **`{python} f"{df_morethan1_hosts_perc:.0f}%"`** of Airbnb hosts letting entire homes in London would be treated as "professional" landlords.
...
-   Comparing this with the annual median income in London,[^6] **£34,100**, on the basis that a host earning this much could, in theory, treat Airbnb rentals as their sole employment.

[^6]: The median income for London taxpayers was £34,100 [(London Datastore, 2023)](https://data.london.gov.uk/dataset/average-income-of-tax-payers-borough-2g1nq/).

```{python}
#| label: Computing median earnings for single and multiple listing hosts over the last 365 days 
#| echo: false
#| warning: false
#| output: false
...
```{python}
#| label: Computing income earnings comparison
#| echo: false
#| warning: false
#| output: false
...
# How many properties does a single-listing host need to have to earn comparably to the median working adult? 
single_listing_host_90d = 34100 / median_income_single_90d 

# How does this change if the property was frequently rented out? 
single_listing_host_90dmore = 34100 / median_income_single_90dmore
```

```{python}
#| label: Computing median earnings for private room hosts over the last 365 days 
#| echo: false
#| warning: false
#| output: false
...
Based on estimated revenues,[^7] the median annual earnings per property were:

[^7]: Estimated revenues from Inside Airbnb, computed as a function of price and occupancy estimates. The median provides a more accurate measure of central tendency for revenues, given the skew created by luxury homes.

-   **£`{python} f"{median_income_single_90d:,.0f}"`** for single-listing hosts and **£`{python} f"{median_income_multi_90d:,.0f}"`** for multi-listing hosts on ["standard lets"]{.underline}.

-   **£`{python} f"{median_income_single_90dmore:,.0f}"`** for single-listing hosts and **£`{python} f"{median_income_multi_90dmore:,.0f}"`** for multi-listing hosts on ["frequent lets"]{.underline}.
...
plt.yticks()
plt.ylabel(" £ per property (Last 365 days)")

# Add title 
fig.suptitle("Figure 6: Median annual revenues per property for frequent lets were \nmuch closer to median household incomes compared to standard lets",
    x= 0.02, y = 0.99,
    fontweight="bold",
    ha="left")
...
Using the more conservative income from single-listing hosts on standard lets, **£`{python} f"{median_income_single_90d:,.0f}"`**, **`{python} f"owning {math.floor(single_listing_host_90d):,} properties"`** is the threshold to make more than the London median income.

However, with frequent lets, **`{python} f"owning {math.ceil(single_listing_host_90dmore):,} properties"`** is sufficient to make near double the median income of London. Therefore, categorising any host owning two or more entire properties as a “professional” landlord is defensible.

If the opposition’s proposal were implemented using this definition, **`{python} f"{len(df_morethan1):,}"`** properties would be subject to increased council tax, or **`{python} f"{df_morethan1_perc:.0f}%"`** of all entire homes listings.

These figures do not account for a landlord's operating costs, which reduces bottom lines, especially when they own fewer properties and cannot benefit from economies of scale.

#### [Who are London's "professional" landlords?]{.underline}

```{python}
#| label: Breakdown of profile of hosts UK and Non UK
#| echo: false
#| warning: false
#| output: false
...
Most of both single **`{python} f"({only1prop_UK_per:.0f}%)"`** and multi-listing **`{python} f"({propmorethan1_UK_per:.0f}%)"`** hosts are UK-based. However, at least one of the hosts that owns over 100 properties is a company based abroad (Dubai). Three others have names suggesting that they are companies. However, these numbers are incomplete because **`{python} f"{host_NA:.0f}%"`** of hosts do not provide a host location. Hence, a dialogue with Airbnb around their registration process may be beneficial.

## 3. Evaluating the opposition's proposal

#### [Is council tax an appropriate additional control?]{.underline}

It is not clear that revenues from increasing council tax would outweigh the negative impact of the high proportion of Airbnbs in London.

All properties in the same band, based on valuation in 1991, pay the same rate of council tax, unless an exemption applies. Council tax is not a mechanism designed to assess current commercial use or income, making it a blunt instrument for targeted market intervention.

The opposition do not explain how the council tax system might be adjusted. However, the recent 2025 Budget outlines plans to modify the council tax system from 2028 to apply a surcharge on luxury properties, using 2026 valuations [@mcnamee2025]. This suggests greater flexibility in the system than has historically been possible, and further work to “piggyback” on these changes could be explored.

To illustrate potential revenues from council tax adjustments, a **2%, 5% and 10%** **premium** was applied to the current council tax Band D rate[^8] for all properties operated by "professional" landlords.

[^8]: Band D sits in the middle of valuation bands and best represents a typical tax without unfairly skewing towards low or high property values [@mhclg2025a].

```{python}
#| label: Simulation of revenue increase for council tax
#| echo: false
#| warning: false
#| output: false
...
```{python}
#| label: Simulation of revenue increase for council tax (2) 
#| echo: false
#| warning: false
#| output: false
...
summary_table = summary_table.rename(columns={
    "Revenue Increase (London Wide)": "Increase (London-wide)",
    "Revenue Increase (Inner London Only)": "Increase (Inner London only)"
})
summary_table = summary_table.reset_index()
summary_table = summary_table.drop(columns=["index"])
```
...
fig.text(
    ...
    ("Source: London Datastore, Council tax charges and bands by borough.\n"
     "Note: A conservative range of 2–10% was chosen for simulation, since councils "
     "proposing a >5% increase must hold a \nsuccessful local referendum "
     "(Ministry of Housing and Local Government, 2025–26)."
    ),
    ha="left",
    va="bottom",
    fontsize=11
...
This indicates a potential yield of between **`{python} f"£{total_2/1_000_000:.1f}m"`** and **`{python} f"£{total_10/1_000_000:.1f}m"`**. Inner London boroughs account for **`{python} f"£{inner_2/1_000_000:.1f}m"`** to **`{python} f"£{inner_10/1_000_000:.1f}m"`** of this total.

The Mayor could therefore justify the policy as one specifically taxing central areas, where commercial exploitation of housing stock is most acute, rather than a blanket tax rise across London ([Figure 7]{.underline}). A targeted surcharge affecting “professional” Airbnb hosts (in the manner of the Budget’s “Mansion Tax” surcharge) is likely to see far less pushback than a general increase in council tax for all properties.

However, determining which bands should be affected is not straightforward, as properties of all types and values are available for short-term let on Airbnb and other platforms.

```{python}
#| lst-label: Plot spatial revenue distribution of increase in council tax
#| echo: false
#| warning: false
scenario = "Band_D_5"
...
# Title
fig.suptitle(
    "Figure 7: Spatial concentration of revenues allows option for justification\n"
...
# Get a point inside Inner London
label_point = inner_boundary_dissolved.geometry.representative_point().iloc[0]

# Optional manual offset (tune these values)
x_offset = 14000   # metres if CRS is EPSG:27700
y_offset = -4000
...
Given that some Airbnb hosts already operate as companies, and that many more would be required to register as “professional” landlords under these proposals, business rates represent a viable targeted alternative that builds on existing regulatory infrastructure rather than creating a parallel scheme. Business rates apply where a property is used for commercial purposes beyond a prescribed threshold [@gov.uk]. This approach offers greater clarity, as "professional" hosts would be classified as businesses removing the need for a separate administrative system. However, business rates are triggered by different thresholds, and additional work would be required to align these with planning permission requirements.

Alternatively, a “[tourist tax]{.underline}”, could be considered. This could raise **£240 million a year** and would not necessarily reduce visitor numbers [@jaffer2025]. This would be more targeted and easily administered, without relying on property valuations or enforcement against non-compliant landlords, since it is typically paid by the tourist.

## 4. Weighing up the way ahead: pros, cons and social mobility

The opposition’s proposal provides an opportunity to emphasise previous steps taken and leverage bipartisan support to implement progressive tax changes.

However, an overly heavy-handed approach risks undermining London’s thriving tourism industry, which contributes **£10 billion a year** to London’s economy and supports around **7% of London’s jobs** [@mayoroflondon2025]. Moreover, London's short letting market is already more tightly regulated than many European equivalents, and overcorrection should be avoided ([Figure 8]{.underline}).

```{python}
#| label: Computing a cities based comparison of current entire-homes ownership 
#| echo: false
#| warning: false
#| output: false
...
```{python}
#| lst-label: Plotting Figure for International Comparison
#| echo: false
#| warning: false
...
# Add title 
fig.suptitle("Figure 8: London's regulatory stance on short-term lets already stricter \nthan many European counterparts and this is reflected in host listings",
    x= 0.02, y = 0.96,
    fontweight="bold",
    ha="left")
...
Furthermore, introducing administrative complexity and costs through broad council tax increases could curb social mobility.

For ordinary Londoners, hosting provides a meaningful supplement to household income, helping to offset rising living costs. Older women living alone and women with disabilities increasingly rely on hosting as a flexible financial lifeline [@remap2022]. For many renting out a single room, earnings are modest: the median annual revenue is around **`{python} f"£{median_home_standard:,.0f}"`** for "standard lets", rising to around **`{python} f"£{median_home_frequent:,.0f}"`** for "frequent lets", often alongside operating costs and tax liabilities on income. 

While better-off “professional” hosts may absorb the additional burden, individual hosts may find hosting no longer financially viable, risking market exit and reduced financial security.

Rather than a blanket increase in council tax for all, a targeted strategy, combining a tourist tax with stricter registration and business rates compliance for professional landlords, would be more proportionate and effective. It is better aligned with the Mayor’s aim of reducing harm, without undermining London’s vibrancy or ordinary residents' livelihoods.

## 5. The aide: deflecting a distraction

While the issue of Airbnb’s coming to the fore has upsides, the aide's conduct is, at the least, a source of embarrassment for the Mayor's office and, at worst, a criminal offence (under the Prevention of Social Housing Fraud Act 2013). Appropriate steps must be taken to uphold public trust in the Mayor's office.

However:

-   The aide's full circumstances unknown and any criminal prosecution is a matter for the CPS; in some circumstances it is permitted to let social housing [@shelter].

-   Even for multi-property owners, our analysis shows that some hosts may require up to seven properties to reach the London median income. Letting two properties, the aide's income may not meet that threshold.

The opposition is clearly using the aide's circumstances to score political points, but in doing so have put themselves in the position of having to support previous initiatives suggested by the Mayor's office: an opportunity that should not be missed. They should not be permitted to use the aide's alleged misdemeanours to distract from the more salient issues around Airbnb's impact on London, or to suggest that individuals should be barred from legitimate ways of supplementing their income.

## 6. Limitations

#### [**Inside Airbnb dataset**]{.underline} [@insideairbnb] 

Inaccuracy in web-scraping may lead to listing numbers being underestimated during the data cleaning process. We further processed the data to remove listings with duplicate IDs, or more than six null fields, which typically contain entries with no reviews or incomplete information.

The dataset is a snapshot, potentially misrepresenting seasonal or short-term listings.

#### [**Housing stock dataset**]{.underline} [@valuationofficeagency2024]

Using total housing stock to normalise Airbnb listings may misrepresent local conditions, as some areas have fewer Airbnb-eligible dwellings (such as care homes) and others higher vacancy rates. 
