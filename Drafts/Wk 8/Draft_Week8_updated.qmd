...
#| warning: false
#| output: false
#Investigate increase in listings 2021-2025
...
-   From 2021-2025, central areas of London generally saw larger increases. The **`{python} f"Borough of {top_boro}"`** saw the **largest increase in listings `{python} f"Borough of {top_boro_pct_change}%"`** (to reach **`{python} f"{top_boro_2025:,.0f}"`** **listings in 2025** ([Figure 1.2]{.underline}).
...
ax.set_title(
    "Figure 1.5: Airbnb Listings as a Proportion of Housing Stock by Borough (2025)",
    fontsize=14,
    pad=15)
...
#| echo: false
#| warning: false
### Mapping the percentage of listings of housing stock in 2025 using Jenks breaks
import mapclassify
...
MSOA_map_plotter (
    ...
    "Figure 1.6: Airbnb Listings as a Proportion of Housing Stock (MSOA, 2025)",
    "Source: Listings data from Insideairbnb.com, Housing Stock from UK Valuation Office Agency.\nNote: As housing stock data for 2025 was unavailable, the latest available data for 2024 was used."
)
```
...
-   Notwithstanding the sensitivity of the assumptions in Inside Airbnb's methodology[^4], we expect this estimate to be a conservative one, given reported **likelihood of multiple listings of the same property**, which some owners have used to circumvent the 90 day rule[^5]. *\[AS note: we should add some concluding remark that brings this back to the issue of "control"?\]*
...
Entire_homes, Entire_homes_pct = type_summary_2025.loc["Entire home/apt", "count"], type_summary_2025.loc["Entire home/apt", "percentage"]
Private_rooms, Private_rooms_pct = type_summary_2025.loc["Private room", "count"], type_summary_2025.loc["Private room", "percentage"]
Shared_rooms, Private_rooms_pct = type_summary_2025.loc["Shared room", "count"], type_summary_2025.loc["Shared room", "percentage"]
Hotel_rooms, Private_rooms_pct = type_summary_2025.loc["Hotel room", "count"], type_summary_2025.loc["Hotel room", "percentage"]
```
...
```{python}
# Count 2025 entire home listings inside each MSOA polygon
gdf_entire25_MSOA = gpd.sjoin(entire_homes_2025_gdf, MSOA[['ecode', 'geometry']], how='left', predicate='within')
MSOA = MSOA.merge(gdf_entire25_MSOA.groupby('ecode').size().rename('entire_listings_2025'), on='ecode', how='left')
MSOA['entire_listings_2025'] = MSOA['entire_listings_2025'].fillna(0).astype(int)

# Count 2025 entire listings as a proportion of total listings
MSOA['entire_prop2025'] = (MSOA['entire_listings_2025'] / MSOA['listings2025']) * 100
MSOA['entire_prop2025'] = MSOA['entire_prop2025'].replace([np.inf, -np.inf], np.nan).fillna(0)
```

```{python}
#| echo: false
#| warning: false
### Mapping the percentage of listings that are entire homes in 2025 using Jenks breaks
import mapclassify
from matplotlib.patches import Patch
...
MSOA_map_plotter (
    ...
    "Figure 2.1.2: Proportion of Airbnb Listings that are entire homes (MSOA, 2025)",
    "Source: Listings data from Insideairbnb.com")
```

#### 2.2 What is the difference between a professional landlord and a commercial landlord? Does this matter for analysis?

*\[I wrote the below before I realised that Owen had done an analysis of this point in his separate doc. I'm leaving it in as some of it may be helpful. I think the below has the possible benefit that it makes the point re business rates, which we may want to come back to as part of our recommendations, and also refers to the details of the question.\]*
...
# Titles and labels  
fig.suptitle("Figure 2.2 Distribution of Listings by Host Portfolio Size \nand Estimated Occupancy (2025)",
              fontsize=14, y=0.94)
...
```{python}
### Mapping percentage of listings that are professionally operated and entire homes
...
MSOA_map_plotter (
    ...
    "Figure 2.3: Proportion of Airbnb Listings that are \n professionally operated entire homes (MSOA, 2025)",
    "Source: Listings data from Insideairbnb.com"
)
```
...
# Ok, so it seems that there were 86 locations with multiple listings from the same host, there were up to 10 listings in each location, which could be plausible if they owned 10 homes at each location. Given the data limitations, we shall not pursue this line of enquiry further. 

# Calculate statistics for hosts with 1, more than 1, and 100 or more entire homes on Airbnb
...
An analysis of host listings[^6] shows that a **small number of hosts are responsible for a disproportionately large number of listings ([Figure 2.2]{.underline})**. Previous studies have found that such "multi-listing hosts" are significant drivers of increase in rent and Airbnb's profit in London[^7].
...
-   A small number, **`{python} f"{df_morethan100_hosts:,} ({df_morethan100_hosts_perc:.2f}%)"`** of hosts, [own over 100 properties]{.underline} but still make up **`{python} f"{df_morethan100_perc:.0f}%"`** of the total number of entire homes available.
...
#| echo: false
#| warning: false
# Computing median earnings for single and multiple listing hosts over the last 365 days 
...
Based on estimated revenues from Inside Airbnb data[^8],

[^8]: Owen to insert a footnote to flesh out assumptions for data cleaning and compilation.

-   The median single-listing host renting out their home for less than 90 days earned **£`{python} f"{median_income_single_90d:,.0f}"`**, while the median multi-listing host earned **£`{python} f"{median_income_multi_90d:,.0f}"`** a year per property.

-   This rises much further when we look at [frequent hosts (i.e. occupancy ≥90 days)]{.underline}. The median single-listing host earned **£`{python} f"{median_income_multi_90d:,.0f}"`**, while multi-listing hosts earned **`{python} f"{median_income_multi_90dmore:,.0f}"`** a year per property.

*[AS note: add a concluding sentence explaining the findings as regards comparison with median income and whether therefore considered "professional"?]*

```{python}
#| echo: false
#| warning: false
## Distribution of revenue per property for standard lets and frequent lets hosts
# Helper function to drop NaN and zeros
def clean(series):
...
# Summarise to *host-level total revenue* for a given mask
def host_total_revenue(mask):
    temp = df.loc[mask, ["host_id", "estimated_revenue_l365d"]]
    temp = temp[temp["estimated_revenue_l365d"] > 0]
    return (
        temp.groupby("host_id")["estimated_revenue_l365d"]
            .sum()
    )
...
a = host_total_revenue(single & less90)   # Single <90 days
b = host_total_revenue(multi  & less90)   # Multi <90 days
c = host_total_revenue(single & more90)   # Single ≥90 days
d = host_total_revenue(multi  & more90)   # Multi ≥90 days

labels = ["Single <90 days","Multi <90 days",
          "Single ≥90 days","Multi ≥90 days"]
...
# Colors: Standard (blue), Frequent (red)
colors = ["skyblue", "skyblue", "tomato", "tomato"]
...
# Dotted separator between (b) and (c)
plt.axvline(x=2.5, color="grey", linestyle="--", linewidth=1)

# Group labels
plt.text(
    1.5, plt.ylim()[0] - (plt.ylim()[1] * 0.1),
    "Standard Lets",
    ha="center", va="top", fontsize=12
)

plt.text(
    3.5, plt.ylim()[0] - (plt.ylim()[1] * 0.1),
    "Frequent Lets",
    ha="center", va="top", fontsize=12
)
...
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
plt.ylabel("Total Revenue per Host (Last 365 days)", fontsize=12)
plt.title(
    "Distribution of total revenue per host by portfolio size \n"
    "and occupancy group (2025)",
    fontsize=14
)
...
# Remove hosts with zero revenue if desired:
host_summary = host_summary[host_summary["total_revenue_l365d"] > 0]
import matplotlib.pyplot as plt
import matplotlib.ticker as ticker
...
plt.scatter(
    host_summary["listing_count"],
    host_summary["total_revenue_l365d"],
    alpha=0.4,
    s=30,
    color="steelblue"
)
...
plt.ylabel("Total Revenue per Host (Last 365 Days)", fontsize=12)
plt.title("Host Total Revenue vs. Number of Listings", fontsize=14)

# Thousands separators on y-axis
plt.gca().yaxis.set_major_formatter(
    ticker.FuncFormatter(lambda x, p: format(int(x), ","))
)
...
#| echo: false
#| warning: false
# Use 'host_location' column in the dfs to compare whether hosts who own > 1 property are more likely to be based outside the UK. Note that there are NA values in some host_location rows; these have been excluded (method for doing so queried with CoPilot) 
...
#| echo: false
#| warning: false
# Plot a bar charts to visualise this data 
data = {
