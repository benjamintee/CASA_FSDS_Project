...
# Ok, so it seems that there were 86 locations with multiple listings from the same host, there were up to 10 listings in each location, which could be plausible if they owned 10 homes at each location. Given the data limitations, we shall not puruse this line of enquiry further. 
...
-   A small number, **`{python} f"{df_morethan100_hosts:,} ({df_morethan100_hosts_perc:.2f}%)"`** hosts, [own over 100 properties]{.underline} but still make up **`{python} f"{df_morethan100_perc:.0f}%"`** of the total number of entire homes available.
...
# Also color medians, whiskers, and caps to match (optional but nicer)
for i, color in enumerate(colors):
...
########################################
# Built the above from Alice's code below, retaining for reference and disucssion. 
