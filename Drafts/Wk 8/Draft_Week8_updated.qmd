...
Opposition critics have suggested that the growth and spread of Airbnbs in London has accelerated, with professional landlords rapidly accumulating properties, flouting rental restrictions to make a profit. This chapter provides a systematic analysis of the situation. 
...
```{python}
#AS note: add echo/warning/output?
...
-   From 2021-2025, central areas of London generally saw larger increases. The Borough of Westminster saw the **largest increase (close to 40%)** to reach **10,588 listings in 2025** ([Figure 1.2]{.underline}).
...
entire_homes_2025_per = round(entire_homes_2025_n / listings_2025 * 100, 0)
...
#| echo: false
#| warning: false
import matplotlib.pyplot as plt #AS note: imported above? can remove if so.
...
*\[I wrote the below before I realised that Owen had dont an analysis of this point in his separate doc. I'm leaving it in as a suggested alternative text. I think the below has the possible benefit that it makes the point re business rates, which we may want to come back to as part of our recommendations, and also refers to the details of the question.\]*
...
#### 2.3 How many hosts have listings for two or more entire properties (homes / apartments)? What is the distribution of listings for such hosts?
...
#AS note: Steps 1 - 3 now aren't named; remove "Step 4"?
# Step 4: Compute % of listings in each bracket
...
plt.barh(dist.index, dist["<90 days"], color=colors[0], label="< 90 days")
plt.barh(dist.index, dist["≥90 days"],
         left=dist["<90 days"], color=colors[1], label="≥ 90 days")

# Add % labels on the stacks
for i, (low, high) in enumerate(zip(dist["<90 days"], dist["≥90 days"])):
    plt.text(low / 2, i, f"{low:.1f}%", va="center", ha="center", fontsize=11)
    plt.text(low + high / 2, i, f"{high:.1f}%", va="center", ha="center", fontsize=11)

plt.xlabel("Percentage of Listings (%)")
plt.ylabel("Host Portfolio Size")
plt.title("Distribution of Listings by Host Portfolio Size\nand Estimated Occupancy (2025)")
#AS note: this plot ultimately needs to be numbered using the same formatting (fig x.y as in section 1. Fig 2.2?)
plt.xlim(0, 50.5)
plt.legend(title="Occupancy Category", loc="upper right")
...
#AS note: I haven't really followed up on the same_location_same_host object, but I've looked at the IDs of the hosts owning  > 100 properties a bit lower down.
...
-   `{python}len(df_only1)` hosts own only 1 entire property. The remaining `{python}len(df_morethan1)` own two or more properties.
...
*AS note: I double checked the format for embedded values in the commentary using CoPilot*

#### 2.4 What is the average income from these properties, and how does it compare with median / average incomes in London?

*\[From Owen's week7 doc:\]* The London Datastore reported that the mean income in London for 2022-2023 was £59,000 and the median was £34,100. *\[expand source?\]*
...
```{python}
#use 'host_location' column in the dfs to try to compare whether hosts who own > 1 property are more likely to be based outside the UK. Note that there are NA values in some host_location rows; these have been excluded (but the numbers of these are noted below).
...
On the basis that any individual owning two or more properties is a "professional" landlord, this would mean that `{python}len(propmorethan1UK)` properties would need to be registered.
