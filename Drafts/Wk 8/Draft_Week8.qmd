...
-   Since 2017, London planning rules state that *entire homes* let on platforms such as Airbnb should not be used for short term letting for more than **90 nights per calendar year**, without planning permission obtained for change of use. This prevents permanent housing from being converted into de facto holiday accommodation.

-   To estimate potential breaches, we look at the subset of listings that are "entire homes / apartments". In 2025, this was XXXX (XX%). Using the“estimated_occupancy_l365d” field, which estimates how many nights the listing was booked in the last 365 days, provides a useful proxy for annual use where raw daily calendar data are unavailable. We treat an entire home as a **likely breach** if "estimated_occupancy_l365d" \> 90.

-   Using this definition:

    -   **12,082 entire home listings** are estimated to be used for more than 90 nights per year.

    -   Close to **2 in 10 of all entire homes listings**, and **around 1 in 8 of all listings in London.**

-   This estimate is likely a conservative one, as we are unable to fully detect multiple listings of the same property, which owners could use to circumvent the 90 day rule and would not be captured by the proxy.
