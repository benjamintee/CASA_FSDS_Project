...
# Dropping rows missing more than 5 values which may be problematic 
...
```{python}
#create a map of listings/stock using continuous colour scheme
fig, ax = plt.subplots(1, 1, figsize=(10, 8))
...
#classify boroughs by inner and outer London boroughs
inner_london = {
...
# Create colour list
colors = ['tab:blue' if borough in inner_london else 'tab:orange' 
          for borough in sorted_categories]

#plotting the bar chart of bnb % of stock by boro
...
#legend plotting
inner_patch = mpatches.Patch(color='tab:blue', label='Inner London')
outer_patch = mpatches.Patch(color='tab:orange', label='Outer London')
...
### Downloading Airbnb Listings from 2021-2025
...
### Cleaning Airbnb Listings from 2021-2025

```{python}
# Evaluating the number of rows with NA in id and other columns 
na_summary = {}
...
### Visualising the Growth in Airbnb Listings from 2021-2025
...
# Create a manually positioned colourbar
...
At the borough level, computing the change in listings relative to change housing stock over the last five year periods.
...
# Axis labels & title 
ax.set_xticklabels(df_bar["name"], rotation= 90, ha="right", fontsize=14)
ax.set_ylabel("Ratio of increase in listings to housing stock", fontsize=14)
ax.tick_params(axis="y", labelsize=16)
ax.set_title(
...
ax.legend(handles, ["Inner London Boroughs", "Outer London Boroughs"], fontsize=15)
...
Airbnb as a proportion of housing stock (By Borough)
...
ax.set_xticklabels(df_bar["name"], rotation=90, ha="right", fontsize=14)
ax.tick_params(axis="y", labelsize=14)
ax.set_ylabel("Listings as percentage of Housing Stock (%)", fontsize=16)
ax.set_title(
...
Airbnb as a proportion of housing stock (By MSOA)
...
```{python}
# Create a spatial plot of listings/stock using Jenks breaks
import mapclassify
from matplotlib.patches import Patch
...
# Titles and annotations
ax.set_title("Figure 6: Airbnb Listings as a Proportion of Housing Stock (MSOA, 2025)", fontsize=19, pad=15)
ax.axis("off")
fig.text(
    0.03, -0.02,
    "Source: Listings data from Insideairbnb.com, Housing Stock from UK Valuation Office.\n"
...
standard = df["occupancy_group"] == "<90 days"
frequent = df["occupancy_group"] == "≥90 days"

# Define origin categories
def origin_category(x):
    if pd.isna(x):
        return "Unknown"
...
df["origin_group"] = df["host_location"].apply(origin_category)

# Build a tidy dataset with 4 groups × 3 origin categories
group_labels = [
    ("Single (Standard)" , single & standard),
    ("Single (Frequent)" , single & frequent),
    ("Multi (Standard)"  , multi  & standard),
    ("Multi (Frequent)"  , multi  & frequent),
]

rows = []
for name, mask in group_labels:
    subset = df.loc[mask]
    total_hosts = subset["host_id"].nunique()

    for cat in ["UK", "Non-UK", "Unknown"]:
        count = subset.loc[subset["origin_group"] == cat, "host_id"].nunique()
        pct = (count / total_hosts * 100) if total_hosts > 0 else 0
        rows.append([name, cat, count, pct])

df_plot = pd.DataFrame(rows, columns=["Group", "Country", "Count", "Percent"])

df_pct  = df_plot.pivot(index="Group", columns="Country", values="Percent")
df_cnt  = df_plot.pivot(index="Group", columns="Country", values="Count")

groups_order = ["Single (Standard)", "Single (Frequent)",
                "Multi (Standard)", "Multi (Frequent)"]

df_pct = df_pct.loc[groups_order]
df_cnt = df_cnt.loc[groups_order]
...
x = np.arange(len(groups_order)) * 0.7
bar_width = 0.5
bottom = np.zeros(len(groups_order))

for country in ["UK", "Non-UK", "Unknown"]:
    pct_vals = df_pct[country].values
    count_vals = df_cnt[country].values
...
fig.suptitle(
    "Distribution of Host Origins by Listing Size and Occupancy Category (2025)",
    fontsize=15, y=0.97
)
